...
# charger la classe Interface
source("../R/Interface.R")

# création de l'objet avec K=2
obj <- Interface$new(method = "kmeans", K = 2, scale = TRUE)
...
for (K in 2:4){
    obj <- Interface$new(method="kmeans", K=K, scale=TRUE)
    obj$fit(X_num)
...
obj_test <- Interface$new(method = "kmeans", K = 2, scale = TRUE)

X_num_prep <- obj_test$.__enclos_env__$private$check_and_prepare_X(X_num)
...
obj_scale <- Interface$new(method = "kmeans", K = 2, scale = TRUE)

X_num_prep <- obj_scale$.__enclos_env__$private$check_and_prepare_X(X_num)
X_num_scaled <- obj_scale$.__enclos_env__$private$scale_active_variables(X_num_prep)
...
obj_km <- Interface$new(method="kmeans", K=2, scale=TRUE)
obj_km$fit(X)
...
```{r}
obj_kmodes <- Interface$new(method = "kmodes", K = 2)
...
```{r}
for (K in 2:4){
    obj <- Interface$new(method="kmodes", K=K)
    obj$fit(X_cat)
    cat("K =", K, " - inertie =", obj$get_inertia(), "\n")
}
```
...
for (i in seq_len(n_runs)) {
  obj <- Interface$new(method = "kmodes", K = 2)
  obj$fit(X_cat)
...
```{r}
# préparation des données
obj_test_km <- Interface$new(method = "kmodes", K = 2, scale = FALSE)

X_cat_prep <- obj_test_km$.__enclos_env__$private$check_and_prepare_X(X_cat)
...
obj_kmodes2 <- Interface$new(method = "kmodes", K = 2)
...
obj_kmodes3 <- Interface$new(method = "kmodes", K = 2)
...
obj_kmodes <- Interface$new(method = "kmodes", K = 3)
obj_kmodes$fit(X_cat)
...
```{r}
obj_kprot <- Interface$new(method="kprototypes", K=3, scale  = TRUE, lambda=1)
...
```{r}
# préparation des données
obj_test_kp <- Interface$new(method="kprototypes", K=3, scale=TRUE, lambda=1)
X_mix_prep <- obj_test_kp$.__enclos_env__$private$check_and_prepare_X(X_mix)
...
obj_kprot2 <- Interface$new(method="kprototypes", K=3, scale = TRUE,
  lambda = 1
)
...
obj_kprot3 <- Interface$new(method="kprototypes", K=3, scale  = TRUE, lambda=1)
...
```{r}
for (K in 2:5) {
  obj <- Interface$new(method="kprototypes", K=K, scale=TRUE, lambda = 1)
  obj$fit(X_mix)
  cat("K =", K, " - inertie = ", obj$get_inertia(), "\n")
}
```
...
for (i in seq_len(n_runs)) {
  obj <- Interface$new(method="kprototypes", K=3, scale=TRUE, lambda=1)
  obj$fit(X_mix)
