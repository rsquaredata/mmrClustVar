...
# charger la classe mmrClustVar
source("../R/mmrClustVar.R")

# création de l'objet avec K=2
obj <- mmrClustVar$new(method = "kmeans", K = 2, scale = TRUE)
...
for (K in 2:5){
    obj <- mmrClustVar$new(method="kmeans", K=K, scale=TRUE)
    obj$fit(X_num)
...
obj_test <- mmrClustVar$new(method = "kmeans", K = 2, scale = TRUE)
...
obj_scale <- mmrClustVar$new(method = "kmeans", K = 2, scale = TRUE)
...
```{r}
for (K in 2:5){
    obj <- mmrClustVar$new(method="kmodes", K=K)
    obj$fit(X_cat)
    cat("K =", K, " - inertie =", obj$get_inertia(), "\n")
}
```
...
for (i in seq_len(n_runs)) {
  obj <- mmrClustVar$new(method = "kmodes", K = 2)
  obj$fit(X_cat)
...
```{r}
# préparation des données
obj_test_km <- mmrClustVar$new(method = "kmodes", K = 2, scale = FALSE)
...
obj_kmodes2 <- mmrClustVar$new(method = "kmodes", K = 2)
...
obj_kmodes3 <- mmrClustVar$new(method = "kmodes", K = 2)
...
obj_kmodes <- mmrClustVar$new(method = "kmodes", K = 3)
obj_kmodes$fit(X_cat)
...
obj_kprot <- mmrClustVar$new(method="kprototypes", K=3, scale  = TRUE, lambda=1)
...
```{r}
# préparation des données
obj_test_kp <- mmrClustVar$new(method="kprototypes", K=3, scale=TRUE, lambda=1)
...
obj_kprot2 <- mmrClustVar$new(method="kprototypes", K=3, scale = TRUE,
  lambda = 1
)
...
obj_kprot3 <- mmrClustVar$new(method="kprototypes", K=3, scale  = TRUE, lambda=1)
...
```{r}
for (K in 2:5) {
  obj <- mmrClustVar$new(method="kprototypes", K=K, scale=TRUE, lambda = 1)
  obj$fit(X_mix)
  cat("K =", K, " - inertie = ", obj$get_inertia(), "\n")
}
```
...
for (i in seq_len(n_runs)) {
  obj <- mmrClustVar$new(method="kprototypes", K=3, scale=TRUE, lambda=1)
  obj$fit(X_mix)
